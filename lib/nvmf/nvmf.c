...
spdk_nvmf_tgt_add_referral(struct spdk_nvmf_tgt *tgt,
			   const struct spdk_nvmf_referral_opts *uopts, bool allow_any_host)
{
	struct spdk_nvmf_referral *referral;
...
int
spdk_nvmf_tgt_nqn_compare(const char *subnqn1,
			  const char *subnqn2)
{
	int cmp;


	cmp = strcmp(subnqn1, subnqn2);
	if (cmp) {
		return cmp;
	}
...
spdk_nvmf_tgt_find_referral(struct spdk_nvmf_tgt *tgt, const char *subnqn)
{
	...
	TAILQ_FOREACH(referral, &tgt->referrals, link) {
		if (spdk_nvmf_tgt_nqn_compare(referral->trid.subnqn, subnqn) == 0) {
			return referral;
		}
	}
