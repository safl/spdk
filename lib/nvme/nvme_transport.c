...
int
nvme_transport_ctrlr_enable_interrupt(struct spdk_nvme_ctrlr *ctrlr)
{
	const struct spdk_nvme_transport *transport = nvme_get_transport(ctrlr->trid.trstring);

	assert(transport != NULL);
	if (transport->ops.ctrlr_enable_interrupt != NULL) {
		return transport->ops.ctrlr_enable_interrupt(ctrlr);
	}
...
int
nvme_transport_qpair_get_efd(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair)
{
	const struct spdk_nvme_transport *transport = nvme_get_transport(ctrlr->trid.trstring);

	assert(transport != NULL);
	if (transport->ops.qpair_get_efd != NULL) {
		return transport->ops.qpair_get_efd(qpair);
	}
