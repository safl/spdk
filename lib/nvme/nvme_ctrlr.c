...
int
spdk_nvme_ctrlr_qpair_get_fd(struct spdk_nvme_qpair *qpair)
{
	struct spdk_nvme_ctrlr *ctrlr = qpair->ctrlr;

	return nvme_transport_qpair_get_fd(ctrlr, qpair);
}

int
spdk_nvme_ctrlr_get_adminq_fd(struct spdk_nvme_ctrlr *ctrlr)
{
	return spdk_nvme_ctrlr_qpair_get_fd(ctrlr->adminq);
}
	...
	if (spdk_nvme_cpl_is_error(cpl)) {
		...
		if (ctrlr->opts.enable_interrupts) {
			...
			if (nvme_transport_ctrlr_enable_interrupts(ctrlr) < 0) {
				...
				nvme_ctrlr_set_state(ctrlr, NVME_CTRLR_STATE_ERROR,
