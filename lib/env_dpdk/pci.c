...
int
spdk_pci_device_enable_interrupts(struct spdk_pci_device *dev, int efd_count)
{
	struct rte_pci_device *rte_dev = dev->dev_handle;
	...
	if (dpdk_pci_device_create_interrupt_efds(rte_dev, efd_count)) {
...
spdk_pci_device_get_interrupt_efd_by_index(struct spdk_pci_device *dev, uint32_t index)
{
	if (index == 0) {
		...
		/* Note: The interrupt vector offset starts from 1. In DPDK these are
		 * mapped to efd index 0 onwards.
		 */
