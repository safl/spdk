			...
			struct spdk_blob_md_page **last_page)
{
	...
	*last_page = NULL;
	if (*page_count == 0) {
		assert(*pages == NULL);
		*pages = spdk_malloc(blob->bs->dev->phys_blocklen, 0,
		...
		*page_count = 1;
		...
		assert(*pages != NULL);
		tmp_pages = spdk_realloc(*pages, blob->bs->dev->phys_blocklen * (*page_count + 1), 0);
		if (tmp_pages == NULL) {
			return -ENOMEM;
		}
		(*page_count)++;
...
blob_load_cpl_extents_cpl(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	if (ctx->pages == NULL) {
		...
		ctx->pages = spdk_zmalloc(blob->bs->dev->phys_blocklen, 0,
					  NULL, SPDK_ENV_NUMA_ID_ANY, SPDK_MALLOC_DMA);
	...
	for (i = ctx->next_extent_page; i < blob->active.num_extent_pages; i++) {
		if (blob->active.extent_pages[i] != 0) {
			...
			bs_sequence_read_dev(seq, &ctx->pages[0], lba,
					     bs_byte_to_lba(blob->bs, blob->bs->dev->phys_blocklen),
	  ...
	  spdk_bs_sequence_cpl cb_fn, void *cb_arg)
{
	...
	ctx->blob = blob;
	ctx->pages = spdk_realloc(ctx->pages, bs->dev->phys_blocklen, 0);
	if (!ctx->pages) {
		free(ctx);
		cb_fn(seq, cb_arg, -ENOMEM);
		return;
	...
	bs_sequence_read_dev(seq, &ctx->pages[0], lba,
			     bs_byte_to_lba(bs, bs->dev->phys_blocklen),
			     blob_load_cpl, ctx);
}
...
blob_persist_clear_extents(spdk_bs_sequence_t *seq, struct spdk_blob_persist_ctx *ctx)
{
	...
	lba_count = bs_byte_to_lba(bs, bs->dev->phys_blocklen);
...
{
	uint64_t lba = bs_dev_page_to_lba(blob, cluster_start_page);
			     ...
			     uint64_t io_unit, spdk_bs_user_op_t *op)
{
	...
	memset(ctx->new_cluster_page, 0, blob->bs->dev->phys_blocklen);
	...
	is_valid_range = blob->back_bs_dev->is_range_valid(blob->back_bs_dev,
			 bs_dev_page_to_lba(blob, cluster_start_page),
...
bs_channel_create(void *io_device, void *ctx_buf)
{
	...
	channel->new_cluster_page = spdk_zmalloc(dev->phys_blocklen, 0, NULL, SPDK_ENV_NUMA_ID_ANY,
				    SPDK_MALLOC_DMA);
	 ...
	 struct spdk_bs_load_ctx **_ctx)
{
	...
	bs->pages_per_cluster = bs->cluster_sz / dev->phys_blocklen;
...
bs_write_used_clusters(spdk_bs_sequence_t *seq, void *arg, spdk_bs_sequence_cpl cb_fn)
{
	...
	mask_size = ctx->super->used_cluster_mask_len * ctx->bs->dev->phys_blocklen;
...
bs_write_used_md(spdk_bs_sequence_t *seq, void *arg, spdk_bs_sequence_cpl cb_fn)
{
	...
	mask_size = ctx->super->used_page_mask_len * ctx->bs->dev->phys_blocklen;
	ctx->mask = spdk_zmalloc(mask_size, 0x1000, NULL,
				 SPDK_ENV_NUMA_ID_ANY, SPDK_MALLOC_DMA);
	if (!ctx->mask) {
		bs_load_ctx_fail(ctx, -ENOMEM);
...
bs_write_used_blobids(spdk_bs_sequence_t *seq, void *arg, spdk_bs_sequence_cpl cb_fn)
{
	...
	mask_size = ctx->super->used_blobid_mask_len * ctx->bs->dev->phys_blocklen;
...
bs_load_replay_md(struct spdk_bs_load_ctx *ctx)
{
	ctx->page_index = 0;
	ctx->cur_page = 0;
	ctx->page = spdk_zmalloc(ctx->bs->dev->phys_blocklen, 0,
				 NULL, SPDK_ENV_NUMA_ID_ANY, SPDK_MALLOC_DMA);
	if (!ctx->page) {
		bs_load_ctx_fail(ctx, -ENOMEM);
		return;
...
bs_parse_super(struct spdk_bs_load_ctx *ctx)
{
	...
	ctx->bs->pages_per_cluster = ctx->bs->cluster_sz / ctx->bs->dev->phys_blocklen;
	     ...
	     spdk_bs_op_with_handle_complete cb_fn, void *cb_arg)
{
	...
	ctx->super->md_page_size = dev->phys_blocklen;
	     ...
	     spdk_bs_op_with_handle_complete cb_fn, void *cb_arg)
{
	...
	if (dev->phys_blocklen == 0) {
		dev->phys_blocklen = SPDK_BS_PAGE_SIZE;
	}
	if ((dev->phys_blocklen % dev->blocklen) != 0) {
	...
	ctx->super->md_page_size = bs->dev->phys_blocklen;
...
bs_delete_open_cpl(void *cb_arg, struct spdk_blob *blob, int bserrno)
{
	...
	if (update_clone) {
		ctx->page = spdk_zmalloc(blob->bs->dev->phys_blocklen, 0, NULL, SPDK_ENV_NUMA_ID_ANY,
					 SPDK_MALLOC_DMA);
		if (!ctx->page) {
			ctx->bserrno = -ENOMEM;
			spdk_blob_close(blob, delete_blob_cleanup_finish, ctx);
...
blob_write_extent_page_ready(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	bs_sequence_write_dev(seq, ctx->page, bs_md_page_to_lba(ctx->bs, ctx->extent),
			      bs_byte_to_lba(ctx->bs, ctx->bs->dev->phys_blocklen),
...
bs_load_grow_continue(struct spdk_bs_load_ctx *ctx)
{
	...
	if (ctx->super->io_unit_size == 0) {
		ctx->super->io_unit_size = ctx->super->md_page_size;
	}
	...
	ctx->bs->cluster_sz = ctx->super->cluster_size;
	ctx->bs->total_clusters = ctx->super->size / ctx->super->cluster_size;
	ctx->bs->pages_per_cluster = ctx->bs->cluster_sz / ctx->super->md_page_size;
	if (spdk_u32_is_pow2(ctx->bs->pages_per_cluster)) {
		ctx->bs->pages_per_cluster_shift = spdk_u32log2(ctx->bs->pages_per_cluster);
	}
	ctx->bs->io_unit_size = ctx->super->io_unit_size;
