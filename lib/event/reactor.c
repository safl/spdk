...
reactor_run(void *arg)
{
	...
	while (1) {
		...
		if (spdk_unlikely(g_scheduler_period > 0 &&
			...
			spdk_trace_record(TRACE_SCHEDULING_PERIOD_START, 0, 0, 0);
...
static void
scheduler_reactor_trace(void)
{
	struct spdk_trace_tpoint_opts opts[] = {
		{
			"SCHEDULING_PERIOD_START", TRACE_SCHEDULING_PERIOD_START,
		...
		{
			"CORE_STATS", TRACE_SCHEDULER_CORE_STATS,
		...
		{
			"THREAD_STATS", TRACE_SCHEDULER_THREAD_STATS,
		...
		{
			"MOVE_THREAD", TRACE_SCHEDULER_MOVE_THREAD,
			OWNER_TYPE_THREAD, OBJECT_NONE, 0,
			{
...
SPDK_TRACE_REGISTER_FN(scheduler_reactor_trace, "scheduler", TRACE_GROUP_SCHEDULER)
