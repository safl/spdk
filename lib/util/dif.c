		  ...
		  uint32_t data_offset, uint64_t guard_seed, struct spdk_dif_ctx_init_ext_opts *opts)
{
	...
	if (dif_flags & SPDK_DIF_FLAGS_NVME_PRACT && md_size > _dif_size(dif_pi_format) && dif_loc) {
		SPDK_ERRLOG("PRACT is not supported if metadata size is larger than DIF size and"
			    " DIF is in the first byte.\n");
		return -ENOTSUP;
	}
