...
void
spdk_log_open(spdk_log_cb *logf)
{
	if (logf) {
		struct spdk_log_opts opts = {.logf = logf};
		opts.size = SPDK_SIZEOF(&opts, logf);
...
spdk_log_open_ext(struct spdk_log_opts *opts)
{
	...
	g_log_opts.logf = SPDK_GET_FIELD(opts, logf, NULL);
	  ...
	  const char *format, va_list ap)
{
	...
	if (g_log_opts.logf) {
		g_log_opts.logf(level, file, line, func, format, ap);
		return;
	}
