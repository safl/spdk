...
{
	uint32_t blocklen = spdk_bdev_desc_get_block_size(bdev_io->internal.desc);
...
_bdev_rw_split(void *_bdev_io)
{
	...
	blocklen = spdk_bdev_desc_get_block_size(bdev_io->internal.desc);
	...
	case SPDK_BDEV_IO_TYPE_WRITE:
		if (_is_buf_allocated(bdev_io->u.bdev.iovs)) {
			...
			spdk_bdev_io_get_buf(bdev_io, bdev_rw_split_get_buf_cb,
					     bdev_io->u.bdev.num_blocks *
					     spdk_bdev_desc_get_block_size(bdev_io->internal.desc));
		}
		break;
	case SPDK_BDEV_IO_TYPE_UNMAP:
...
static inline int
bdev_io_init_dif_ctx(struct spdk_bdev_io *bdev_io, struct spdk_bdev *bdev)
{
...
/* We need to allocate bounce buffer if bdev supports memory domains but IO is not
 * aware of metadata.
 */
static inline bool
...
spdk_bdev_desc_get_block_size(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? bdev->blocklen - bdev->md_len : bdev->blocklen;
}
...
spdk_bdev_desc_get_md_size(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? 0 : bdev->md_len;
}
...
spdk_bdev_desc_is_md_interleaved(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? false : spdk_bdev_is_md_interleaved(bdev);
}
...
spdk_bdev_desc_is_md_separate(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? false : spdk_bdev_is_md_separate(bdev);
}
...
spdk_bdev_desc_get_dif_type(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? SPDK_DIF_DISABLE : spdk_bdev_get_dif_type(bdev);
}
...
spdk_bdev_desc_get_dif_pi_format(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? SPDK_DIF_PI_FORMAT_16 : spdk_bdev_get_dif_pi_format(bdev);
}
...
spdk_bdev_desc_is_dif_head_of_md(struct spdk_bdev_desc *desc)
{
	...
	return desc->opts.no_metadata ? false : spdk_bdev_is_dif_head_of_md(bdev);
}
				    ...
				    enum spdk_dif_check_type check_type)
{
	...
	return desc->opts.no_metadata ? false : spdk_bdev_is_dif_check_enabled(bdev, check_type);
}
