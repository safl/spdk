...
reservation_request_cb_fn(struct spdk_bdev_io *bdev_io, bool success, void *ctx)
{
	...
	struct reservation_cb_arg *cb_arg = ctx;

	test_failed = (success != cb_arg->success_expected);

	if (test_failed) {
		SPDK_ERRLOG("expected bdev reservation request failed\n");
	} else {
		SPDK_NOTICELOG("expected bdev reservation request is successful\n");
	}
			  ...
			  bool success_expected)
{
	...
	struct reservation_cb_arg *resv_cb_arg = calloc(1, sizeof(*resv_cb_arg));
...
static void
bdev_read_cb_fn(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	struct bdev_context_t *bdev_context = cb_arg;
...
static void
bdev_write_cb_fn(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
	/* Complete the I/O */
	spdk_bdev_free_io(bdev_io);
...
test_io_operations(void *ctx, bool success_expected)
{
	...
	rc = test_bdev_write(bdev_context);
	if (rc) {
	...
	rc = test_bdev_read(bdev_context);
	if (rc) {
		SPDK_ERRLOG("expected read operation failed\n");
		return rc;
	}

	/* checking if the expected success is achieved or not */
	if ((success_expected && !rc) || (!success_expected && rc)) {
		return 0;
	}
	return -1;
}
