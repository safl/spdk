...
get_thread_stats_current() {
	...
	for thread in "${!thread_map[@]}"; do
		busy[thread]="$((total_busy[thread] - past_busy[thread]))"
		past_busy[thread]="${total_busy[thread]}"
		idle[thread]="$((total_idle[thread] - past_idle[thread]))"
		past_idle[thread]="${total_idle[thread]}"
	done
	xtrace_restore
}
