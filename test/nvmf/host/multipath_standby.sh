...
$rpc_py -s $bdevperf_rpc_sock bdev_nvme_attach_controller -b NVMe0 -t $TEST_TRANSPORT -a $NVMF_FIRST_TARGET_IP -s $NVMF_SECOND_PORT -f ipv4 -n nqn.2016-06.io.spdk:cnode1 -x standby
...
sleep 3
controllers=$($rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | jq -r ".[0] | .ctrlrs[]")
[[ -n $(jq -r "select(.state == \"disabled\") | .trid | select(.trsvcid == \"$NVMF_PORT\")") ]] <<< "$controllers"
[[ -n $(jq -r "select(.state == \"enabled\") | .trid | select(.trsvcid == \"$NVMF_SECOND_PORT\")") ]] <<< "$controllers"

$rpc_py -s $bdevperf_rpc_sock bdev_nvme_attach_controller -b NVMe0 -t $TEST_TRANSPORT -a $NVMF_FIRST_TARGET_IP -s $NVMF_THIRD_PORT -f ipv4 -n nqn.2016-06.io.spdk:cnode1 -x standby
...
controllers=$($rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | jq -r ".[0] | .ctrlrs[]")
[[ -n $(jq -r "select(.state == \"disabled\") | .trid | select(.trsvcid == \"$NVMF_PORT\")") ]] <<< "$controllers"
[[ -n $(jq -r "select(.state == \"disabled\") | .trid | select(.trsvcid == \"$NVMF_SECOND_PORT\")") ]] <<< "$controllers"
[[ -n $(jq -r "select(.state == \"enabled\") | .trid | select(.trsvcid == \"$NVMF_THIRD_PORT\")") ]] <<< "$controllers"
...
sleep 3
controllers=$($rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | jq -r ".[0] | .ctrlrs[]")
[[ -n $(jq -r "select(.state == \"enabled\") | .trid | select(.trsvcid == \"$NVMF_PORT\")") ]] <<< "$controllers"
[[ -n $(jq -r "select(.state == \"disabled\") | .trid | select(.trsvcid == \"$NVMF_SECOND_PORT\")") ]] <<< "$controllers"
[[ -n $(jq -r "select(.state == \"disabled\") | .trid | select(.trsvcid == \"$NVMF_THIRD_PORT\")") ]] <<< "$controllers"
...
cat $testdir/try.txt
# if this test fails it means we didn't change to the second
...
$rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | grep -q NVMe0
controllers=$($rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | jq -r ".[0] | .ctrlrs[]")
[[ -z $(jq -r ".trid | select(.trsvcid == \"$NVMF_THIRD_PORT\")") ]] <<< "$controllers"
...
#detach an active controller and there is standby controller available
...
controllers=$($rpc_py -s $bdevperf_rpc_sock bdev_nvme_get_controllers | jq -r ".[0] | .ctrlrs[]")
[[ -n $(jq -r "select(.state == \"enabled\") | .trid | select(.trsvcid == \"$NVMF_SECOND_PORT\")") ]] <<< "$controllers"
[[ -z $(jq -r ".trid | select(.trsvcid == \"$NVMF_PORT\")") ]] <<< "$controllers"
