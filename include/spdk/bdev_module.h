...
struct spdk_bdev_io {
	 ...
	 */
	struct spdk_bdev_io_stack_frame ctx_stack[0];
 ...
 * it has received, saves its state by calling \ref spdk_bdev_io_push_stack
 * and then calling this function. The completion function is then responsible
 * for calling \ref spdk_bdev_io_pop_stack prior to completing it to the
 * bdev layer.
 *
