...
static void _comp_unmap_subcmd_done_cb(void *ctx, int32_t error);
...
_comp_submit_unmap_split(void *ctx)
{
	...
	if (split_ctx->subcmd_error != 0 ||
		...
		reduce_rw_blocks_cb(bdev_io, split_ctx->subcmd_error);
		free(split_ctx);
		return;
	}
...
static void
_comp_unmap_subcmd_done_cb(void *ctx, int32_t error)
{
	struct compress_unmap_split_ctx *split_ctx = ctx;

	split_ctx->subcmd_error = error;
