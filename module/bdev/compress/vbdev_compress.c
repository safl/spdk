...
struct compress_unmap_split_ctx {
	...
	uint32_t num_full_chunks_consumed;
	uint64_t num_partial_chunks;
	uint32_t num_partial_chunks_consumed;
	/* Used to hold the partial chunk information. There will only be less than two,
	because chunks that cannot be fully covered will only appear at the beginning or end. */
