...
#define CTRLR_STRING(nvme_ctrlr) \
	((nvme_ctrlr->active_path_id->trid.trtype == SPDK_NVME_TRANSPORT_TCP || \
	 nvme_ctrlr->active_path_id->trid.trtype == SPDK_NVME_TRANSPORT_RDMA) ? \
...
bdev_nvme_create_qpair(struct nvme_qpair *nvme_qpair)
{
	...
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Connecting qpair %p started.\n", qpair);
...
bdev_nvme_reset_ctrlr_complete(struct nvme_ctrlr *nvme_ctrlr, bool success)
{
	...
	if (!success) {
		...
		if (bdev_nvme_failover_trid(nvme_ctrlr, false, false)) {
			...
			NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Try the next alternate trid now.\n");
			      ...
			      struct nvme_ctrlr_channel *ctrlr_ch, void *ctx)
{
	...
	if (nvme_qpair->qpair != NULL) {
		NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Start disconnecting qpair %p.\n", nvme_qpair->qpair);
...
bdev_nvme_reset_check_qpair_connected(void *ctx)
{
	...
	if (!spdk_nvme_qpair_is_connected(nvme_qpair->qpair)) {
		return SPDK_POLLER_BUSY;
	}

	NVME_CTRLR_NOTICELOG(nvme_qpair->ctrlr, "qpair %p was connected.\n", nvme_qpair->qpair);
			     ...
			     void *ctx)
{
	...
	if (rc == 0) {
		...
		NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Start checking qpair %p to be connected.\n",
				     nvme_qpair->qpair);
...
{
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Start reconnecting ctrlr.\n");
...
bdev_nvme_reset_io_continue(void *cb_arg, int rc)
{
	...
	NVME_BDEV_NOTICELOG(nbdev, "continue reset_io %p, rc:%d\n", bio, rc);
...
_bdev_nvme_reset_io(struct nvme_io_path *io_path, struct nvme_bdev_io *bio)
{
	...
	if (rc != 0 && rc != -EBUSY) {
		NVME_BDEV_NOTICELOG(nbdev, "reset_io %p could not reset ctrlr %s, rc:%d\n",
				    bio, CTRLR_STRING(io_path->qpair->ctrlr), rc);
		return rc;
	}
	...
	if (rc == -EBUSY) {
		...
		NVME_BDEV_NOTICELOG(nbdev, "reset_io %p was queued to ctrlr %s.\n",
				    bio, CTRLR_STRING(io_path->qpair->ctrlr));
	}
