	...
	if (nbdev_ctrlr != NULL) {
		...
		TAILQ_FOREACH(nctrlr, &nbdev_ctrlr->ctrlrs, tailq) {
			if (nctrlr->opts.multipath != nvme_ctrlr->opts.multipath) {
				/* With controler config consistency check enabled, all controllers with the same
				 * name must be configured the same way, either for multipath or failover.
				 * If the configuration doesn't match - report error.
				 */
				rc = -EINVAL;
				goto exit;
			}
...
discovery_poller(void *arg)
{
	...
	if (ctx->detach_ctx) {
		...
		/* With controler config consistency check enabled, all controllers must be
		 * configured explicitely either for multipath or failover. While discovery use multipath
		 * mode, we need to set this in bdev options as well.
		 */
